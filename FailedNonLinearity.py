...
def plot_decision_boundary(model, X, y):
    ...
    # 1. Define the plot area (meshgrid)
    # Add some padding to the min and max data values
    x_min, x_max = X[:, 0].min() - 0.1, X[:, 0].max() + 0.1
    y_min, y_max = X[:, 1].min() - 0.1, X[:, 1].max() + 0.1
    xx, yy = np.meshgrid(np.linspace(x_min, x_max, 101),
                         np.linspace(y_min, y_max, 101))

    # 2. Pass every point on the grid to the model
    with torch.inference_mode():
        # Flatten the data (ravel) to the format the model expects
        X_to_pred = torch.from_numpy(np.column_stack((xx.ravel(), yy.ravel()))).float()
        y_logits = model(X_to_pred)
        y_pred = torch.round(torch.sigmoid(y_logits))

    # 3. Plot the result
    y_pred = y_pred.reshape(xx.shape)  # Reshape back to grid form
